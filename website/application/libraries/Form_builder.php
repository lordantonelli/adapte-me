...
class Form_builder {
	...
	'default_radio_class' => 'radio',
	'default_checkbox_class' => 'checkbox',
	'default_control_label_class' => 'col-sm-3 control-label',
	'default_form_control_class' => 'col-sm-9',
	'default_form_class' => 'form-horizontal col-sm-12',
	'default_button_classes' => 'btn btn-primary',
	'default_date_post_addon' => '<span class="input-group-addon"><i class="fa fa-calendar"></i></span>',
	'default_color_post_addon' => '<span class="input-group-addon"><i></i></span>',
...
//        }
	...
	if ($this->func == 'form_combine') {
		...
		if ($counter > 0) {
		    if(!empty($elm_options_backup['combine_divider'])){
			$input_html_string .= $elm_options_backup['combine_divider'];
		    } else {
			$this->elm_options['class'] = isset($this->elm_options['class']) ? $this->elm_options['class'] . ' ml-lg' : 'ml-lg';
		    }
		}
	    ...
	    if(!isset($this->elm_options['id'])){
		$this->elm_options['id'] = $this->elm_options['name'];
	    }
	    $this->elm_options['aria-describedby'] = $this->elm_options['id'].'-error';
	    /*
	     * json
	     * button (anchor, a)
	     * label
		...
		case 'form_a':
		    ...
		    $href = '';
		    if(isset($this->elm_options['action'])){
			$href = $this->elm_options['action'];
		    }elseif (isset($this->elm_options['href'])) {
			$href = $this->elm_options['href'];
		    }

		    $value = $this->elm_options['label'];
		    unset($this->elm_options['label']);

		    $input_html_string = anchor($href, $value, $this->elm_options);
		    break;
		case 'form_label':
		...
		case 'form_date':
		    ...
		    $config = (isset($this->elm_options['config'])) ? $this->elm_options['config'] : '';
		    $CI->set_javascript_inline('$("#'.$this->elm_options['id'].'").datepicker({' . $config . '});', 'datepicker-'.$this->elm_options['id']);
		    $CI->set_javascript_inline('$("#'.$this->elm_options['id'].'").mask("99/99/9999");', 'mask-'.$this->elm_options['id']);
		    unset($this->elm_options['config']);
		    
		    $this->input_addons['exists'] = true;
		    $this->input_addons['post_html'] = $this->config['default_date_post_addon'];
		    //$this->elm_options['type'] = 'date';
		    $this->elm_options['data-date-format'] = 'dd/mm/yyyy';

		    if (empty($this->elm_options['value'])) {
			$this->elm_options['value'] = date('d/m/Y', strtotime('today'));
		    } else {
		...
		case 'form_color':
		    //obtenemos instancia global del core CodeIgniter
		    $CI = & get_instance();
		    $CI->set_css('assets/colorpicker/css/bootstrap-colorpicker.min.css', 'datepicker');
		    $CI->set_javascript('assets/colorpicker/js/bootstrap-colorpicker.min.js', 'datepicker');
		    $config = (isset($this->elm_options['config'])) ? $this->elm_options['config'] : '';
		    unset($this->elm_options['config']);
		    $CI->set_javascript_inline('$("#'.$this->elm_options['id'].'").colorpicker({' . $config . '});', 'colorpicker-'.$this->elm_options['id']);
		    
		    $this->input_addons['exists'] = true;
		    $this->input_addons['post_html'] = $this->config['default_color_post_addon'];
		    
		    //$this->elm_options['type'] = 'color';
		    $input_html_string = form_input($this->elm_options);
		    break;
		case 'form_input':
		    $input_html_string = form_input($this->elm_options);
		...
		case 'form_dropdown':
			...
			$CI =& get_instance();
			$CI->set_javascript('assets/select2/js/select2.min.js', 'select2');
			$CI->set_javascript_inline('$("select").select2();', 'select2');
			$CI->set_css('assets/select2/css/select2.min.css', 'select2');
		...
		case 'form_radio':
		    ...
		    if (isset($this->elm_options['options']) && !empty($this->elm_options['options'])) {
			$name = $this->elm_options['name'];
			$id = (!isset($this->elm_options['id'])) ? $name : $this->elm_options['id'];
			$options = $this->elm_options['options'];
			$select_default = $this->elm_options['value'];
			...
			if (!empty($this->config['default_radio_class'])) {
			    $class = str_replace($this->config['bootstrap_required_input_class'], '', $this->elm_options['class']);
			    /* add class="valid" to all dropdowns (makes them not full width - and works better with select2 plugin) */
			    if (strpos($class, $this->config['default_radio_class']) === FALSE) {
				$class .= ' ' . $this->config['default_radio_class'];
			    }
			    $this->elm_options['class'] = trim($class);
			}
			$num_id = 1;
			foreach ($options as $key => $value) {
			    $select = ($key == $select_default) ? TRUE : FALSE;
			    $input = form_radio($name, $key, $select, 'id="' . $id.$num_id . '"');   
			    $input_html_string .= form_label($input . $value, $id.$num_id, $this->elm_options);
			    $num_id ++;
			}
		    } else {
			dump($this->elm_options);
			show_error('Tried to create `form_radio` with no options. (id="' . $this->elm_options['name'] . '")');
		    }
		    break;
		case 'form_checkbox':
		    /* form_dropdown is different than an input */
			...
			if (!empty($this->config['default_checkbox_class'])) {
			    $class = str_replace($this->config['bootstrap_required_input_class'], '', $this->elm_options['class']);
			    /* add class="valid" to all dropdowns (makes them not full width - and works better with select2 plugin) */
			    if (strpos($class, $this->config['default_checkbox_class']) === FALSE) {
				$class .= ' ' . $this->config['default_checkbox_class'];
			    }
			    $this->elm_options['class'] = trim($class);
			}
			$num_id = 1;
			foreach ($options as $key => $value) {
			    $select = ($key == $select_default) ? TRUE : FALSE;
			    $input = form_checkbox($name, $key, $select, 'id="' . $id.$num_id . '"');   
			    $input_html_string .= form_label($input . $value, $id.$num_id, $this->elm_options);
			    $num_id ++;
			}
		    } else {
	    ...
	    if(isset($this->elm_options['id'])){
		$input_html_string .= form_error($this->elm_options['id']);
	    }
	}
	$ret_string = '';
	$ret_string .= ($include_pre_post) ? $this->_pre_input() : '';
	$ret_string .= $this->_build_input_addons_pre();
	if($this->func == 'form_date' || $this->func == 'form_color'){
	    $ret_string = str_replace('input-group"', 'input-group col-sm-4"', $ret_string);
	}
	...
	$class = $this->config['default_input_container_class'];
	if(isset($this->elm_options['id'])){
	    $class .= has_error($this->elm_options['id']);
	}
	...
	if ($this->func == 'form_date') {
	    return '<div class="date date-picker ' . $this->config['default_form_control_class'] . '" data-date="' . $this->elm_options['value'] . '" data-date-format="yyyy-mm-dd" data-date-viewmode="years">';
	}
	...
	if (!isset($this->elm_options['label']) || $this->elm_options['label'] == 'none') {
	    return '<div class="'.$this->config['default_control_label_class'].'"></div>'; /* the keyword none */
	...
	if($this->func != 'form_radio' || $this->func != 'form_checkbox'){
	    $name = '';
	}
	...
	return form_label($label, $name, array(
